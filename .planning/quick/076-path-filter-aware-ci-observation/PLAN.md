...
Accepted: `node --test src/skills/dough-execute-plan/scripts/ci-revision-coverage-stop-states.test.mjs src/skills/dough-execute-plan/scripts/ci-revision-coverage-not-required-shutdown.test.mjs src/skills/dough-execute-plan/scripts/ci-mailbox.test.mjs src/skills/dough-execute-plan/scripts/ci-not-required-guidance.test.mjs` passed (12/12). New shutdown test drives four ignored-only descendants B1-B4 whose respective ancestors A1-A4 are pending/incomplete/success/failure: B1/B2 (open ancestors) retain `{state:"not_required", basis:{sha, state:"pending"|"incomplete"}}` in `terminal.coverage.unproved`; B3/B4 (terminal ancestors) are fully omitted; A4's `CI_FAILURE` is still delivered exactly once; no `not_required` revision ever triggers `CI_DISCOVERY_DELAYED`. New guidance test (`ci-not-required-guidance.test.mjs`, following this directory's `ci-custom-guidance.test.mjs` convention for testing published reference prose) asserts `ci-monitor.md`'s new `not_required` paragraph names GitHub-default-only scope, rejects invented success and spurious missing-run warnings, names `basis.sha`/`basis.state`, and distinguishes pending/incomplete from success/failure. Custom-adapter regression: `node --test src/skills/dough-execute-plan/scripts/ci-command-adapter.test.mjs src/skills/dough-execute-plan/scripts/ci-command-adapter-failures.test.mjs src/skills/dough-execute-plan/scripts/ci-command-adapter-unavailable.test.mjs src/skills/dough-execute-plan/scripts/ci-custom-guidance.test.mjs src/skills/dough-execute-plan/scripts/ci-custom-host-bridge.test.mjs` — 19/19 pass, files untouched. Full regression: `node --test --test-concurrency=1 src/skills/dough-execute-plan/scripts/*.test.mjs` — 184/184 pass. `npm run lint` clean. Implementation: a `not_required` revision's `basis` now refreshes every poll to `{sha, state}` (never sticky — an ancestor found pending earlier and now terminal is not left stale; an ancestor that cannot currently be resolved leaves prior recorded evidence alone rather than erasing it), using `attemptState()` extracted for reuse between `observedRevision` and this resolution. `ci-mailbox-store.mjs`'s `unresolvedRevisions` now also includes a `not_required` revision whose `basis.state` isn't yet `success`/`failure`, reporting `{sha, state, basis}`; a proved-terminal one stays fully omitted, unchanged. `ci-monitor.md` gained a guidance paragraph (AGENTS.md's behavior-review checklist applied: invocation context scoped to `state:"not_required"` GitHub-default records; required context names the exact `basis.sha`/`basis.state` fields the runtime produces; useful outcome walked concretely for both a pending-ancestor and a failed-ancestor descendant, each tied to a passing test assertion) — edited at its `src/skills/dough-execute-plan/references/` source location only, per this repository's own AGENTS.md rule not to hand-edit installed `.claude/skills/`/`.agents/skills/` copies.

Learning: a cancelled ancestor's real state is now visible at shutdown (`basis.state:"incomplete"`), but no live `CI_INCOMPLETE` event is emitted mid-poll to its ignored-only descendants while polling continues — only pending/success/failure ancestors feed the live failure-acquisition path slice 3 built. This is a deliberate reading of this slice's literal proof text ("retains pending/incomplete evidence with the applicable source" at shutdown, not a new live event), consistent with slice 3's Learning leaving that exact question open for this slice; recorded here as the final resolution rather than left further open, since this is the last planned slice.

Story-level acceptance: re-checked against the plan's Outcome paragraph and Required-behavior items 1-5 (see "Scope and current decisions") — all five are delivered across the four slices, and slice 4 closes the final gap (a still-open applicable ancestor no longer silently vanishes from terminal reporting at shutdown, and a proved one is cleanly omitted, matching the Outcome's "no synthetic green or ambiguous missing-run report").
