...
git_publication_admission_correction_prompt() {
  local where=$1
  printf '%s\n' \
    "Use this project's installed Open Dough guidance. The execution retrospective of the completed story SEED-R#publish-notes found that release notes are listed in file-name order, and wrote its bounded follow-up correction in the originating checkout without publishing it: the story ${NATIVE_ADMISSION_IDENTITY} (seeds/R.md#order-notes) and its correction plan quick/R/PLAN.md. No backlog list holds this work yet. The developer accepts this correction for execution and asks you to take ownership of it now so the team sees it in Taken; implementation happens in a later session, so do not change product code. ${where} Report the outcome."
}
...
run_admission_correction_counterexamples() {
  ...
  printf '%s\n' 'story-plan: quick/R/PLAN.md' 'entry-plan: quick/R/PLAN.md' \
