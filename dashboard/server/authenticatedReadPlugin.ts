...
// (`./authenticatedRead.ts`). Mounts the same middleware in dev and preview,
// and tears owned `gh` subprocesses down through `closeServer`/
...
export function authenticatedReadPlugin(): Plugin {
  ...
  return {
    name: "dough-authenticated-read",
    configureServer(server) {
      cleanup = installAuthenticatedReadMiddleware(server.middlewares);
    },
