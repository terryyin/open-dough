...
check in this story, and deferred fewer installer runs per promise to a later
story, SEED-037#fewer-installer-runs-per-promise, since delivered and
recoverable from `43e306c`. The anchor keeps its original name because it is
...
- Proving an installation or update promise with fewer installer runs,
  ...
  `story-payload-update.sh`. This is a coverage decision, delivered by
  SEED-037#fewer-installer-runs-per-promise.
...
isolated test as the result. Per-platform reduction stays deferred as its own
decision.
...
installer runs to a separate story, since delivered. Reducing per-platform runs
remains open.
