...
Behavior: Given the named candidate and saved Codex evidence → review applicability
and run only uncovered checks → the maintainer has a supported Codex verdict.
Use `tests/dough-adr-awareness-codex-delivery-to-use.sh` for a missing combined
journey, subject to the entry condition. Include review, retained evidence, and
fixture cleanup in this single proof loop; no duplicate installed-use run when
the journey already proves it.
...
Cursor verdict. Use `tests/dough-adr-awareness-cursor-delivery-to-use.sh` for a
missing combined journey, subject to the entry condition. Retain and review the
attempt and its cleanup within this proof loop.
...
`tests/dough-adr-awareness-claude-delivery-to-use.sh` for a missing combined journey,
subject to the entry condition. Retain and review the attempt and its cleanup
within this proof loop.

### 4. Record whether the candidate is accepted
Type: Behavior
...
| --- | --- | --- | --- | --- |
| Slice 1 | Codex | Pending candidate-specific review; Quick 014 is a starting point. | Pending candidate-specific review. | Pending |
| Slice 2 | Cursor | Pending candidate-specific review; Quick 014 is a starting point. | Pending candidate-specific review. | Pending |
| Slice 3 | Claude Code | Pending candidate-specific review; Quick 014 is a starting point. | Pending candidate-specific review. | Pending |
| Slice 4 | Overall | Confirm coverage and candidate identity from the three rows. | Confirm shared cases resolve the candidate's behavioral risks. | Pending |

## Sizing and readiness

Target approximately five minutes of active work per leaf. One focused native
journey can exceed that in external runtime; record that exception rather than
splitting update from fresh use. For a selected journey, use the existing
...
**Refinement recommended: slices 1–3**, once the candidate and unresolved checks
are known. Their current execution path cannot yet be certified as target-sized.
Slice 4 is a short evidence review. This is a bounded conditional plan, not a
claim of readiness for native execution.
