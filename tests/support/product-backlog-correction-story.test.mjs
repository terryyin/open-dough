...
const planPath = "slice-plans/070-order-release-notes/PLAN.md";
const story = {
...
test("a backlog listing a story's plan as separate work is refused unchanged", async (t) => {
  const duplicated = backlogOf(
    [takenEntry],
    [`- [Old copy](slice-plans/057-script-product-backlog/PLAN.md)`, ...queued],
  );
...
test("a plan-homed correction keeps its recorded identity and is listed through its plan", async (t) => {
  const legacyPlan = "slice-plans/060-repair-the-release-notes/PLAN.md";
  const identity = "slice-plans/060-repair-the-release-notes";
  const project = scratchProject(t);
  projectFile(
    project,
    legacyPlan,
