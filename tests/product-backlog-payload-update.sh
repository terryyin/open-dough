...
# physical entry root, and that project backlog bytes are untouched. The
...
# represents both. Protection of edited or removed managed files, which one
# install.sh declaration drives for every file, is owned by
# story-payload-update.sh.
