...
test("an admitted planned mission and a new correction story close in turn, keeping the unfinished sibling", async (t) => {
  ...
  const correctionPlan = ".planning/quick/R/PLAN.md";
  ...
  const closed = await closeInTrunkMode(
    ...
    late.identity,
    {
      cleanup(workspace) {
        ...
        rmSync(join(workspace, ".planning/quick/late"), { recursive: true });
  ...
  assert.ok(await remoteText(trunk, tip, ".planning/quick/A/PLAN.md"));
  ...
  const last = await closeInTrunkMode(
    ...
    {
      cleanup(workspace) {
        rmSync(join(workspace, ".planning/quick/R"), { recursive: true });
        const path = join(workspace, seedA);
        writeFileSync(
          path,
