#!/usr/bin/env bash
# Snapshot decisive fixture paths and hashes. Usage: snapshot-fixture.sh <fixture> <dest>
...

{
  ...
  hash_one planning/plans/remove-extra-greeting-prefix.md
...
(cd -- "${FIXTURE}" && find . \
  -path './.git' -prune -o \
  -type f -print0 | sort -z | xargs -0 shasum -a 256) > "${DEST}/full-file-hashes.txt"
