...
| **Story or requirement** | The problem is understood but several useful outcomes are possible | Ordered Valuable, Visible, Vertical stories |
| **Execution leaf** | One story's goal, scope, and key examples are understood | Behavior leaves and only immediately enabling Structure leaves |
...
## Decompose execution leaves

Use execution leaves only after one story is understood and executable planning
is authorized. Every leaf is exactly one of:
...
resolution, but do not split tests from the Behavior they prove or create leaves
around files, layers, components, specialists, or activities.

Refine a leaf when it has independent postconditions or proof loops, separable
...
## Size and escalate execution leaves

Use the adopting project's execution-leaf target, hard limit, exceptions, and
repeated-overrun threshold. Include implementation, focused verification, and
leaf-local cleanup in the sizing hypothesis. Resolve missing limits before
...
judgment when ownership is unclear. Renaming, retrying, or splitting a leaf does
not erase an overrun.

After a completed leaf, update the same plan when evidence changes only its
remaining leaves. When evidence changes the story's goal, examples, scope, or
...
before another leaf-only refinement. Record delivered value, remaining scope,
