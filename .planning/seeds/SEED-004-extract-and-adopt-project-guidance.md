...
<a id="focused-recoverable-execution-context"></a>

### 21. Keep plan execution context focused and recoverable

**Status:** Refined; scope accepted on 2026-09-14; slice planning authorized.

**Plan:** [045 — Focused, recoverable execution context](../quick/045-focused-recoverable-execution-context/PLAN.md).

**Goal:** A developer executing a multi-slice story can continue, interrupt,
resume, and change the remaining plan with trustworthy decisions and proof,
while the coordinator accumulates less repeated or irrelevant material from
routine coordination. Reduce the context cost of carrying a story through
execution without sacrificing its product understanding or recoverability.

**Evidence:** The investigation in Codex task
`01a09dc1-bb0b-7153-8168-d8b9ffbcc4f3` examined six coordinator histories across
Doughnut, Pygardon, and Open Dough. Five added roughly 40,000–53,000 input
tokens before the first agent delegation. Doughnut's `Execute plan 118`
(`01a09a7b-c104-73b2-9d67-345fd7abc7bc`) grew from 34,806 to 109,310 input
tokens during execution; its plan-edit commands contained about 5,600 characters
versus about 270,000 characters of coordinator tool output. Pygardon's
`Execute plan 129` (`01a09d53-b219-7b80-af1f-457b56852c50`) reached 183,122
input tokens before delegation while conducting profiling and prerequisite
investigation. Repeated broad reads, truncation followed by overlapping rereads,
and code/proof inspection are supported causes. Plan editing contributes but
is not established as the dominant cause. Later retrospective and story-wrap-up
turns must be excluded when attributing execution cost; encrypted message bodies
prevent an exact allocation to agent reports or private reasoning.

**Established intent:** The developer selected this problem as the highest
priority and requested refinement before implementation. Interruption recovery
and useful continuous decision context, especially for replanning, must both
survive the change. The research findings are evidence, not approved solutions.

**Scope:** Improve how shared plan-execution guidance selects, obtains,
passes, retains, and recovers context throughout one story execution. Address
pre-delegation guidance loading as well as repeated slice cycles; optimizing
plan updates alone cannot explain or address the observed growth.

- Load sufficient project and workflow context for the current responsibility,
  with relevant detail available when a decision requires it. Avoid recovering
  omitted passages by routinely repeating large overlapping document batches.
- Let implementation, refactoring, and bounded investigation return actionable
  conclusions, changed boundaries, proof, and evidence locations. Preserve the
  ability to inspect underlying evidence when acceptance or a conflicting
  observation requires judgment; brevity must not turn an unsupported pass into
  accepted proof.
- Distinguish information needed to decide the next action from information
  needed to recover the execution and from raw operational history. Preserve
  scope, execution identity, work ownership, delivery state, completed proof,
  unresolved decisions, and learnings that constrain remaining work. Do not
  require the coordinator to carry every representation of that information.
- Make normal continuation and recovery consistent: interruption during an
  implementation, refactor, plan update, or delivery step must leave enough
  evidence to reconcile actual files, agents, and Git state and identify the
  next safe action. Missing or contradictory evidence remains an explicit
  recovery decision, never a guessed completion.
- When new evidence invalidates an assumption, retain its decision-relevant
  explanation and consequences, preserve compatible completed work and proof,
  and revise the remaining plan coherently. Context reduction must not suppress
  useful investigation, changed direction, or human-owned decisions.

**Alternatives and selection:** Start by reducing redundant retrieval
and retained detail within the existing execution responsibilities. The current
[disposable research guidance](../../src/skills/dough-execute-plan/references/disposable-research.md)
already calls for isolated investigation, distilled conclusions, and bounded
excerpts; assess why the surrounding workflow does not consistently produce
that behavior before adding another procedure. Merely shortening the main
skill or reducing plan-edit frequency is too narrow. Transferring proof
acceptance or delivery to other agents is deferred. On 2026-09-14 the developer
accepted preserving the coordinator's existing responsibilities while improving
context selection and evidence handling. A new memory service, checkpoint registry,
or forced coordinator replacement is not selected by this refinement.

**Key examples:**

- Given a multi-slice plan and available project guidance, starting execution
  obtains the context needed to authorize and delegate the first slice without
  repeatedly loading the same broad references; later slices obtain newly
  relevant detail without routinely reconstructing the entire starting context.
- Given an implementation return and then a refactor return, the coordinator
  can judge the promised behavior and changed proof boundaries. An incomplete
  or contradictory return causes targeted evidence inspection or a returned
  proof gap, not automatic acceptance to save context.
- Given interruption after edits or during delivery, resumption identifies what
  actually completed, preserves owned and unrelated work, and continues from
  the unfinished boundary without falsely marking a slice done or repeating
  still-valid completed proof.
- Given a later observation contradicting an earlier assumption, the coordinator
  can explain the change, revise affected remaining slices, and retain valid
  completed work; a disputed scope or ADR decision still goes to the human.
- Given an unusually large but necessary investigation, the agent may obtain
  the evidence needed for the decision. No arbitrary token ceiling, log-size
  threshold, or prohibition on compaction truncates the required reasoning.

**Evaluation:** Compare coordinator behavior on a representative
multi-slice execution with the observed baseline patterns: unnecessary repeated
guidance reads, overlapping recovery reads, and raw evidence loaded into the
parent should decrease, while the developer can still trace proof acceptance,
recover the next action after interruption, and understand a changed decision.
Use available input-token measurements as supporting evidence, separating initial
host context, compaction, and later lifecycle work; a smaller final context alone
does not establish improvement. Review the key recovery and replanning examples
without building a permanent benchmark or mandatory per-slice accounting system.
The representative case and comparative observations are specified in plan 045.

**Boundaries:** Preserve required proof quality, independent refactoring,
delivery outcomes, human decision authority, and established Story Branch Mode
semantics. Author any resulting public guidance in `src/skills/`, retaining one
shared behavior for Codex, Cursor, and Claude Code. This story does not promise
lower total multi-agent token consumption, faster wall time, zero compactions,
changes to the Codex host or its tool catalog, or optimization of retrospective
and story wrap-up. Release/adoption work and permanent telemetry infrastructure
are not selected here. Keep necessary project context; missing prerequisites
must remain visible.

**Decision context:** [ADR 0002](../../docs/adrs/0002-software-development-lifecycle-principles-accepted.md)
values low coordination cost and inexpensive changes of direction, while leaving
the allocation of agent duties to workflow guidance. It does not establish a
fixed coordinator topology or token target for this story.

**Decisions:** Preserve coordinator proof acceptance and delivery ownership.
Judge improvement by reduced unnecessary retrieval with preserved decision and
recovery behavior; no fixed numerical reduction target is required. Planning
selects the representative exercise and records the limits of its evidence.

**Depends on:** Existing execution guidance and the retained investigation; no
prerequisite backlog story has been identified. Queueing and refinement do not
authorize slice planning, implementation, or release.
