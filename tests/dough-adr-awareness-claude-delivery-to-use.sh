...
if [[ ${1:-} != '--native' ]]; then
  echo 'PASS: the Claude Code delivery-to-use fixture covers a previous public skill, default-branch improvement, fresh native use, alternate-layout conflicts, platform isolation, and coexistence.'
  exit 0
fi
...
run_native_claude "${update_output}" \
  "Use \$dough-update ${delivery_source_url} to update this adopter from the supplied cloneable fixture source. Follow the installed updater exactly. Report the source, commit, Claude Code as the running tool, every installed path, and that this selects the fixture's current default branch rather than a release or installed version. Do not invoke ADR awareness yet."
