#!/usr/bin/env bash
# Build the disposable wrap-up fixture with unfinished Trim names and an
# existing Formal titles feature-story follow-up. Install current Open Dough
# source for Claude. Does not launch Claude.
# shellcheck disable=SC2312
set -euo pipefail

WORKTREE=/private/tmp/open-dough-quick-039-native-wrap-up-acceptance
EVIDENCE="${WORKTREE}/.planning/quick/039-native-wrap-up-acceptance/evidence/slice-3"
...
cp -- "${FIXTURE_SRC}/planning/plans/formal-titles.md" \
  "${FIXTURE}/planning/plans/formal-titles.md"
...
cmp -s -- \
  ...
  "${FIXTURE}/.claude/skills/dough-product-backlog/SKILL.md"
if [[ -f ${FIXTURE}/.agents/skills/dough-story-wrap-up/SKILL.md ]]; then
  cmp -s -- \
    "${WORKTREE}/src/skills/dough-story-wrap-up/SKILL.md" \
...
git -C "${FIXTURE}" add -A
git -C "${FIXTURE}" commit --quiet -m "Record unfinished Trim names and Formal titles follow-up"
...

{
  ...
  printf 'installed_wrap_up_sha256=%s\n' "$(shasum -a 256 "${FIXTURE}/.claude/skills/dough-story-wrap-up/SKILL.md" | awk '{print $1}')"
