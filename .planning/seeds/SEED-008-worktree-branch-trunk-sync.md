...
```json dough-story-state
{"schemaVersion":1,"refinement":"refined","approach":"planned","plan":"../quick/098-first-claude-ci-attachment/PLAN.md","assessment":"ready","reasons":[],"basis":{"document":"0f3c7d28c0bcb820d8ff0e99f148a28b63ee4e8f08107c1693f06733072d8283","plan":"6163bf8a332e10b988654b9d2eea5bceb6df11536e38a810b77f72890a70e982"}}
```
...
- **Scope:** Use explicit session input when supplied; otherwise obtain the owning Claude coordinator's identity from `CLAUDE_CODE_SESSION_ID`. Carry the same identity through bridge verification and binding, register the actual accepted revision, and reuse the observer on the next increment. Keep publication acceptance and observation availability separate. No identity or unavailable bridge produces an actionable coverage gap, never a fabricated identity or an attached claim.
- **Verified prerequisite:** On 2026-09-25, Claude Code 2.1.282 supplied matching Bash environment and hook identities in fresh interactive, `--print`, and native `--background` sessions. The launcher removed inherited identity and supplied no session-id flag. The [documented variable](https://code.claude.com/docs/en/env-vars) is sufficient for the coordinator; transcript-path guessing is unnecessary. This proves identity availability, not first-delivery CI notification. Older runtime compatibility is not established by this probe.
- **Key examples:** (1) Fresh supported Claude coordinator, no explicit session JSON: first managed increment attaches observation, registers the accepted SHA, and receives its CI event. Cover the supported background invocation as well. (2) A second increment reuses the observer and registers its new SHA. (3) Explicit session input remains authoritative, including owner metadata; ambient identity must not overwrite it. (4) Neither source provides identity, or the bridge is unavailable: report the cause and preserve truthful publication results. (5) Events remain isolated from another coordinator or subagent under existing ownership rules.
- **Excluded:** Accepted-publication replay repairs, cleanup of pre-existing hidden observers, verdict acquisition owned by Plan 097, dashboards, new host adapters, autonomous wake-ups, general session discovery, and complete background execution/wrap-up. Normal first attachment must not create duplicate observers. Background coordinators are in scope; delegating delivery to a subagent or transferring observer ownership is not a new promise.
- **Evaluation:** Exercise first delivery through the real command boundary without fixture-supplied session JSON, then event delivery to the owning Claude hook and second-increment reuse. Supplement deterministic publication/registration proof with fresh native Claude foreground and background observations of the taught invocation. An attached receipt or the earlier identity-only probes cannot substitute for observing the delivered event.
- **Plan:** [098 — First Claude CI attachment](../quick/098-first-claude-ci-attachment/PLAN.md).
